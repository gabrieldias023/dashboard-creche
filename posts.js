...
// O edge /media do graph.instagram.com NÃO aceita prefixo de versão
// (retorna "Unknown path components: /media"). Por isso rodamos sem versão.
const GRAPH_VERSION = process.env.GRAPH_VERSION || '';
const BASE = GRAPH_VERSION ? `${GRAPH_HOST}/${GRAPH_VERSION}` : GRAPH_HOST;
...
async function getUserId() {
  ...
  const body = await graphGet(`/me?fields=id`);
  if (!body.id) throw new Error('não consegui resolver o IG user id (/me?fields=id)');
  return body.id;
}

async function getAllMedia(userId) {
  const all = [];
  let next = `/${userId}/media?fields=id,timestamp,media_type,media_product_type,caption,permalink,like_count,comments_count&limit=50`;
  for (let page = 0; page < 200 && next; page++) {
    ...
    if (nextUrl) {
      ...
      next = u.pathname.replace(GRAPH_VERSION ? `/${GRAPH_VERSION}` : '', '') + '?' + u.searchParams.toString();
    } else {
      next = null;
    }
    await sleep(200);
